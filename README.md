...
- `compress` (boolean, default `false`) - if present, and the server supports the compressed protocol, the packets between the client and the server will be compressed with zlib (deflate). This reduces the network traffic at the cost of some CPU time, so it pays off on slow or metered links, or when large query results travel the network. The compression starts after the authentication. When used together with `tls`, the packets are compressed before being encrypted.
