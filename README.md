<!--
	...
	deno run --allow-all https://raw.githubusercontent.com/jeremiah-shaulov/tsa/v0.0.57/tsa.ts doc-md --outFile=README.md --outUrl=https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md --importUrl=https://deno.land/x/office_spirit_mysql@v0.28.0/mod.ts mod.ts
-->
...
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-p9mn>/' > /tmp/example-p9mn.ts
// deno run --allow-env --allow-net /tmp/example-p9mn.ts

import {MyPool} from 'https://deno.land/x/office_spirit_mysql@v0.28.0/mod.ts';
...
> {<br>
> &nbsp; &nbsp; 📄 `readonly` [dsn](generated-doc/interface.MyPoolOptions/README.md#-readonly-dsn-dsn--string)?: [Dsn](generated-doc/class.Dsn/README.md) | `string`<br>
> &nbsp; &nbsp; 📄 `readonly` [maxConnsWaitQueue](generated-doc/interface.MyPoolOptions/README.md#-readonly-maxconnswaitqueue-number)?: `number`<br>
> &nbsp; &nbsp; 📄 `readonly` [onLoadFile](generated-doc/interface.MyPoolOptions/README.md#-readonly-onloadfile-onloadfile)?: [OnLoadFile](generated-doc/type.OnLoadFile/README.md)<br>
> &nbsp; &nbsp; 📄 `readonly` [onBeforeCommit](generated-doc/interface.MyPoolOptions/README.md#-readonly-onbeforecommit-onbeforecommit)?: [OnBeforeCommit](generated-doc/type.OnBeforeCommit/README.md)<br>
> &nbsp; &nbsp; 📄 `readonly` [managedXaDsns](generated-doc/interface.MyPoolOptions/README.md#-readonly-managedxadsns-dsn--string--readonlyarraydsn--string)?: [Dsn](generated-doc/class.Dsn/README.md) | `string` | ReadonlyArray\<[Dsn](generated-doc/class.Dsn/README.md) | `string`><br>
> &nbsp; &nbsp; 📄 `readonly` [xaCheckEach](generated-doc/interface.MyPoolOptions/README.md#-readonly-xacheckeach-number)?: `number`<br>
> &nbsp; &nbsp; 📄 `readonly` [logger](generated-doc/interface.MyPoolOptions/README.md#-readonly-logger-logger)?: [Logger](generated-doc/interface.Logger/README.md)<br>
> }
...
- `allowPublicKeyRetrieval` (boolean, default `false`) - if the server uses `caching_sha2_password` or `sha256_password` authentication over unencrypted TCP connection, this library can need the server RSA public key to encrypt the password. `sha256_password` always needs it, and `caching_sha2_password` needs it when the server asks for the full authentication - that happens when the password is not in the server side cache, like on the first connection after the server restart. The client cannot see the state of that cache, so such connection is refused before the authentication starts, rather than only when the server happens to ask for the key. If this parameter is present, the key will be requested from the server itself, through the untrusted connection. This is vulnerable to man-in-the-middle attacks, where the attacker can substitute the key, and decrypt the password. To avoid the risk, enable `tls`, or pin the trusted key in `serverPublicKey`, or connect through Unix-domain socket.
...
- `compress` (boolean or string, default `false`) - if present, and the server supports the compressed protocol, the packets between the client and the server will be compressed. This reduces the network traffic at the cost of some CPU time, so it pays off on slow or metered links, or when large query results travel the network. There are 2 algorithms: zlib (deflate), that every server supports, and zstd (usually better and faster), that only MySQL 8.0.18+ supports (and the runtime must have zstd in `node:zlib` - Deno 2.7+). The plain `compress` negotiates the best of what's supported: zstd if possible, else zlib. `compress=zlib` and `compress=zstd` pin the algorithm (if the server doesn't support the pinned one, the connection is not compressed). `compress=zstd:N` also sets the zstd compression level (1 - 22, default 3), that both sides will use. The compression starts after the authentication. When used together with `tls`, the packets are compressed before being encrypted.
...
- `caching_sha2_password` - the default since MySQL 8.0. On unencrypted TCP connections the full authentication requires the server RSA public key, so such connections are refused unless the key can be pinned or retrieved - see `allowPublicKeyRetrieval` and `serverPublicKey`
...
To encrypt the traffic, add the [tls](generated-doc/class.Dsn/README.md#-accessor-tls-boolean) parameter to the DSN:
...
If the server has a self-signed certificate, or a certificate issued by your private CA, provide the CA certificate in the [tlsCaCert](generated-doc/class.Dsn/README.md#-accessor-tlscacert-string) parameter:

```ts
import {MyPool, Dsn} from 'https://deno.land/x/office_spirit_mysql@v0.28.0/mod.ts';
...
If the certificate is issued to a different host name than the one you connect to (for example, you connect by IP address, or through an SSH tunnel), specify the certificate's host name in [tlsHostname](generated-doc/class.Dsn/README.md#-accessor-tlshostname-string).
...
If there was no free slot during the [connectionTimeout](generated-doc/class.Dsn/README.md#-accessor-connectiontimeout-number) period, or if the waiting queue is full ([maxConnsWaitQueue](generated-doc/interface.MyPoolOptions/README.md#-readonly-maxconnswaitqueue-number) items long), exception is thrown.
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-65ya>/' > /tmp/example-65ya.ts
...
> ⚙ MyConn.[query](generated-doc/class.MyConn/README.md#-querycolumntypecolumnvaluesql-sqlsource-params-params-queryoptions-queryoptions-resultsetspromiserecordstring-columntype)\<ColumnType=[ColumnValue](generated-doc/type.ColumnValue/README.md)>(sql: [SqlSource](generated-doc/type.SqlSource/README.md), params?: [Params](generated-doc/type.Params/README.md), queryOptions?: [QueryOptions](generated-doc/private.type.QueryOptions/README.md)): [ResultsetsPromise](generated-doc/class.ResultsetsPromise/README.md)\<Record\<`string`, ColumnType>><br>
...
You can read all the rows with [Resultsets.all()](generated-doc/class.Resultsets/README.md#-all-promiserow), [ResultsetsPromise.all()](generated-doc/class.ResultsetsPromise/README.md#-all-promiserow), [Resultsets.buffered()](generated-doc/class.Resultsets/README.md#-buffered-promisethis) or [ResultsetsPromise.buffered()](generated-doc/class.ResultsetsPromise/README.md#-buffered-promiseresultsetsrow).

```ts
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-pf4z>/' > /tmp/example-pf4z.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-ksv8>/' > /tmp/example-ksv8.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-rlut>/' > /tmp/example-rlut.ts
...
- [MyConn.query()](generated-doc/class.MyConn/README.md#-querycolumntypecolumnvaluesql-sqlsource-params-params-queryoptions-queryoptions-resultsetspromiserecordstring-columntype) method iterates over rows as Javascript default objects with fields.
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-ajdy>/' > /tmp/example-ajdy.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-7bvr>/' > /tmp/example-7bvr.ts
// deno run --allow-env --allow-net /tmp/example-7bvr.ts

import {MyPool, ColumnValue} from 'https://deno.land/x/office_spirit_mysql@v0.28.0/mod.ts';
import {assertEquals} from 'jsr:@std/assert@1.0.19/equals';
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-dbu0>/' > /tmp/example-dbu0.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-2dm1>/' > /tmp/example-2dm1.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-aymx>/' > /tmp/example-aymx.ts
// deno run --allow-env --allow-net /tmp/example-aymx.ts

import {Dsn, MyPool} from 'https://deno.land/x/office_spirit_mysql@v0.28.0/mod.ts';
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-10go>/' > /tmp/example-10go.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-1f17>/' > /tmp/example-1f17.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-855m>/' > /tmp/example-855m.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-t6u5>/' > /tmp/example-t6u5.ts
...
> ⚙ MyConn.[prepare](generated-doc/class.MyConn/README.md#-preparecolumntypecolumnvaluesql-sqlsource-queryoptions-queryoptions-promiseresultsetsrecordstring-columntype)\<ColumnType=[ColumnValue](generated-doc/type.ColumnValue/README.md)>(sql: [SqlSource](generated-doc/type.SqlSource/README.md), queryOptions?: [QueryOptions](generated-doc/private.type.QueryOptions/README.md)): Promise\<[Resultsets](generated-doc/class.Resultsets/README.md)\<Record\<`string`, ColumnType>>>
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-wr6k>/' > /tmp/example-wr6k.ts
...
> ⚙ MyConn.[prepare](generated-doc/class.MyConn/README.md#-preparecolumntypecolumnvaluesql-sqlsource-queryoptions-queryoptions-promiseresultsetsrecordstring-columntype)\<ColumnType=[ColumnValue](generated-doc/type.ColumnValue/README.md)>(sql: [SqlSource](generated-doc/type.SqlSource/README.md), queryOptions?: [QueryOptions](generated-doc/private.type.QueryOptions/README.md)): Promise\<[Resultsets](generated-doc/class.Resultsets/README.md)\<Record\<`string`, ColumnType>>><br>
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-np7j>/' > /tmp/example-np7j.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-7xum>/' > /tmp/example-7xum.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-h9zl>/' > /tmp/example-h9zl.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-khw0>/' > /tmp/example-khw0.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-rjwg>/' > /tmp/example-rjwg.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-c5vl>/' > /tmp/example-c5vl.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-mbya>/' > /tmp/example-mbya.ts
// deno run --allow-env --allow-net --allow-write /tmp/example-mbya.ts

import {MyPool, SqlLogToWritable} from 'https://deno.land/x/office_spirit_mysql@v0.28.0/mod.ts';
import {assertEquals} from 'jsr:@std/assert@1.0.19/equals';
...
You can see [here](https://github.com/jeremiah-shaulov/office_spirit_mysql/blob/v0.21.1/private/sql_log_to_writable.ts) how [SqlLogToWritable](generated-doc/class.SqlLogToWritable/README.md) class is implemented,
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-y5hf>/' > /tmp/example-y5hf.ts
...
Instead of `startTrx()` you can use [conn.getTrx()](generated-doc/class.MyConn/README.md#-gettrxoptions-readonly-boolean-xaid-string-xaid1-string-promisetrx). It starts a transaction the same way (and accepts the same `options`), but returns a [Trx](generated-doc/class.Trx/README.md) object that you can use together with `await using`.
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-jrt0>/' > /tmp/example-jrt0.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-hwat>/' > /tmp/example-hwat.ts
...
// To download and run this example:
// export DSN='mysql://root:hello@localhost/tests'
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/office_spirit_mysql/v0.28.0/README.md' | perl -ne 's/^> //; $y=$1 if /^```(.)?/; print $_ if $y&&$m; $m=$y&&$m+/<example-w4qu>/' > /tmp/example-w4qu.ts
